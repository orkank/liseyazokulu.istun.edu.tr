  ...
  public function readySQL($validate = false) {
    ...
    for($key=0;$key<sizeof($this->data);$key++) {
      ...
      if($is_column) {
          ...
          */
          $this->readySQL['columns'][$keys[$key]] = ((isset($this->data[$keys[$key]]['int']) AND $this->data[$keys[$key]]['int'] == 1 AND !empty($this->data[$keys[$key]]['value']))?
          $this->data[$keys[$key]]['value']:"'{$this->data[$keys[$key]]['value']}'");
      } else {
        ...
        if(is_array($this->data[$keys[$key]]['value'])) {
          $this->readySQL['nodes'][$keys[$key]] = "'".serialize($this->sl->encode($_POST[$keys[$key]])). "'";
        } else {
          ...
          switch (@$inputs[$e]['type']) {
            case 'accordions':
              if(isset($this->data[$inputs[$e]['slug']])) {
                $this->sl->smarty->assign('accordion_data', $this->sl->decode(unserialize($this->data[$inputs[$e]['slug']])));
              } else
                $this->sl->smarty->assign('accordion_data', array());

              $this->sl->smarty->assign('input',$inputs[$e]);
              $value[] = $this->sl->smarty->fetch('accordions.html', microtime());
            break;
            case 'parents':
            /*
            ...
            */
            ...
            case 'templates':
              $inputs[$e]['options'] = $this->sl->dir($this->sl->settings['theme_path'] .DS. 'templates');
              ...
              for($s=0;$s<sizeof($this->data[$input['slug'] . '{'.$this->sl->languages('langs')[$z]['prefix'].'}']);$s++) {
                $items[] = str_replace(
                  ...
                  array(
                    ...
                    '<img src="'.$this->data[$input['slug'] . '{'.$this->sl->languages('langs')[$z]['prefix'].'}'][$s].'">',
                    $input['slug'] .'{'. $this->sl->languages('langs')[$z]['prefix'].'}[]'
                  ),
                  $template['item']
                );
