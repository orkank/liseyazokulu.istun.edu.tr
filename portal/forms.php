...
$array = array(
  ...
  array('mdi mdi-file-document-box', 'Başarılı Ödemeler', 'regulasr_form', 2),
  array('mdi mdi-file-document-box', 'Tamamlanmayan Kayıtlar', 'regular_forzm', 1),
);
...
if (isset($sl->post['action']) and $sl->post['action'] == 'excel') {
  ...
  // $sheet->setCellValue('C1', 'Programlar');
  // $sheet->setCellValue('D1', 'Veli Adı');
  // $sheet->setCellValue('E1', 'Veli Soyadı');
  // $sheet->setCellValue('F1', 'Veli E-posta');
  // $sheet->setCellValue('G1', 'Veli Telefon');
  // $sheet->setCellValue('H1', 'Öğrenci T.C. Kimlik');
  // $sheet->setCellValue('I1', 'Öğrenci Adı');
  // $sheet->setCellValue('J1', 'Öğrenci Soyadı');
  // $sheet->setCellValue('K1', 'Öğrenci E-posta');
  // $sheet->setCellValue('L1', 'Öğrenci Telefon');
  // $sheet->setCellValue('M1', 'Doğum Tarihi');
  // $sheet->setCellValue('N1', 'Öğrenci Okulu');
  // $sheet->setCellValue('O1', 'Öğrenci Devam Edilen Sınıf');
  // $sheet->setCellValue('P1', 'Öğrenci İl');
  // $sheet->setCellValue('Q1', 'Öğrenci İlçe');
  // $sheet->setCellValue('R1', 'Öğrenci Adres');
  // $sheet->setCellValue('S1', 'Öğrenci Programlar');
  // $sheet->setCellValue('T1', 'Öğrenci Sözleşme');
  ...
  */
  ...
  $array = array_map(function ($e) use ($sl) {
    $e['json'] = json_decode($e['json'], true);
    $programs = implode(',', $e['json']['program_id']);
    $programs = $sl->db->QueryArray("SELECT `name{tr}` AS `name`, `id` FROM `contents` WHERE `id` IN ({$programs})");
    $total = 0;

    $programs = array_map(function($e) use ($sl, &$total) {
      $price = $sl->db->QuerySingleValue("SELECT `value` FROM `content_nodes` WHERE `cid`='{$e['id']}' AND `key`='price{tr}'");
      $total += intval(str_replace('.', '', $price)); // 2.00;
    ...
    $data = [
      ...
      $e['json']['veliEmail'],
      $e['json']['veliTelefon'],
      $e['json']['ogrenciTcKimlikNo'],
      $e['json']['ogrenciAd'],
      $e['json']['ogrenciSoyad'],
      $e['json']['ogrenciEmail'],
      $e['json']['ogrenciTelefon'],
      $e['json']['birthDay'] . ' ' . $e['json']['birthMonth'] . ' ' . $e['json']['birthYear'],
      $e['json']['okul'],
      $e['json']['devamEdilenSinif'],
      $e['json']['il'],
      $e['json']['ilce'],
      $e['json']['adres'],
      $programs,
      ($e['json']['agreement'] == 'on') ? 'Evet' : 'Hayır',
    ];
  ...
  $array = array_merge([
    ['ID Numarası', 'Kayıt Tarihi', 'Toplam Tutar', 'Programlar', 'Veli Adı', 'Veli Soyadı', 'Veli E-posta', 'Veli Telefon', 'Öğrenci T.C. Kimlik', 'Öğrenci Adı', 'Öğrenci Soyadı', 'Öğrenci E-posta', 'Öğrenci Telefon',
    'Doğum Tarihi', 'Öğrenci Okulu',
    'Öğrenci Devam Edilen Sınıf', 'Öğrenci İl', 'Öğrenci İlçe', 'Öğrenci Adres', 'Öğrenci Programlar', 'Öğrenci Sözleşme']
  ], $array);
...
// echo $sql;
  ...
  $dt->edit('program', function ($data) use ($sl) {
    $data['json'] = json_decode($data['json'], true);
    $program_ids = implode(',', $data['json']['program_id']);
    $programs = $sl->db->QueryArray("SELECT `name{tr}` AS `name` FROM `contents` WHERE `id` IN ({$program_ids})");
    $programs = array_map(function($e) {
      return "<span class=\"badge badge-primary m-1\">{$e['name']}</span>";
    }, $programs);
  ...
  $dt->edit('type', function ($data) {
    ...
    $uniq = ($data['status'] == 1) ? '<span class="badge badge-info ml-1">Ödenmemiş</span>' : '<span class="badge badge-primary ml-1">Ödenmiş</span>';
    $status = $status . $uniq;

    $value = '
    <a data-filter="#content-for-load" data-width="600" data-height="600" data-href="javascript:;"
href="forms.view.php?id=' . $data['id'] . '"
data-_fancybox="wcallback" data-type="ajax">Görüntüle ' . $status . ' </a>
    ';
