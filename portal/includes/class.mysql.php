...
class MySQL {
	...
	public function __construct($connect = true, $database = null, $server = null,
								$username = null, $password = null, $port = null, $charset = null, $pcon) {
	...
	public function BuildSQLUpdate($tableName, $valuesArray, $whereArray = null) {
		$sql = "";
		//$valuesArray = self::Escape($valuesArray);
	...
	public function Escape($singleOrArray) {
		if(!is_array($singleOrArray))
			return mysqli_real_escape_string($this->mysql_link, $singleOrArray);

		$keys = array_keys($singleOrArray);

		for($i=0;$i<sizeof($singleOrArray);$i++)
			$singleOrArray[$keys[$i]] = mysqli_real_escape_string($this->mysql_link, $singleOrArray[$keys[$i]]);
	...
	*/
	...
	public function Open($database = null, $server = null, $username = null,
		...
		$this->mysql_link = @mysqli_connect(
			$this->db_host, $this->db_user, $this->db_pass, $this->db_dbname, $this->port);

			//var_dump($this->mysql_link);
			//echo gettype($this->mysql_link);
	 ...
	 *                Values can be: MYSQL_ASSOC, MYSQLI_NUM, MYSQLI_BOTH
	 * @return array A multi-dimensional array containing all the data
	 *               returned from the query or FALSE on all errors
	 */
	public function QueryArray($sql, $resultType = MYSQLI_BOTH, $use_key = false) {
		$this->Query($sql);
		if (! $this->Error()) {
			return $this->RecordsArray($resultType,$use_key);
		} else {
			return false;
		}
	}
	 ...
	 */
	public function RecordsArray($resultType = MYSQLI_BOTH, $use_key = false) {
		$this->ResetError();
		// Changed added to line below ($this->RowCount() > 0 && $this->GetColumnCount() > 0)
		if ($this->last_result) {
	...
	public function SelectDatabase($database, $charset = "") {
		...
		if (! (mysqli_select_db($this->mysql_link, $database))) {
			...
			if ((strlen($charset) > 0)) {
				...
				/*
				* Changed! From line 1441 to 1452
				* SET NAMES and COLLATION_CONNECTION was added because utf8 code page is not working without thats.
				*/
				if (! (mysqli_query($this->mysql_link, "SET NAMES '$charset'"))) {
					$this->SetError();
					$return_value = false;
				}
				if (! (mysqli_query($this->mysql_link, "SET COLLATION_CONNECTION = 'utf8_general_ci'"))) {
					$this->SetError();
					$return_value = false;
				}
			}
	 ...
	 */
	static public function SQLFix($source) {
		if(is_array($source)) {
			foreach($source as $key => $value)
				if (is_string($source)) $source[$key] = @addslashes($value);
			return $source;
	 ...
	 * @return string
	 */
	static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT ,$max = false) {
		$return_value = "";
	...
	public function TransactionBegin() {
		...
		if (! $this->IsConnected()) {
			...
			if (! $this->in_transaction) {
				if (! mysqli_query($this->mysql_link, "START TRANSACTION", $this->mysql_link)) {
					$this->SetError();
					return false;
				} else {
	...
	public function TransactionEnd() {
		...
		if (! $this->IsConnected()) {
			...
			if ($this->in_transaction) {
				if (! mysqli_query($this->mysql_link, "COMMIT", $this->mysql_link)) {
	...
	public function TransactionRollback() {
		...
		if (! $this->IsConnected()) {
			...
			if(! mysqli_query($this->mysql_link, "ROLLBACK", $this->mysql_link)) {
